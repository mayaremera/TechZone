  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response = await fetch("http://localhost:8080/products?page=1&per_page=60", {
          headers: {
        ...
        const formattedProducts = data.map((product) => {
          // Correctly access the first image URL (images is an array of strings)
          const firstImageUrl =
            product.images && product.images.length > 0
              ? product.images[0] // Directly use the string
              : "https://placehold.co/300x300"; // Updated placeholder URL
          return {
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      ...
      try {
        ...
        // Update wishlist to match the expected structure
        const formattedWishlist = data.map((item) => ({
          product_id: item.product_id,
          product_name: item.product_name,
          price: item.price,
          image_url: item.images && item.images.length > 0 ? item.images[0] : "https://placehold.co/300x300",
        }));
  ...
  return (
    <section className="py-5 px-4 md:px-16">
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        <div
          className={`xl:w-1/4 ${isMobileFilterOpen ? "block" : "hidden"} xl:block transition-all duration-300`}
        >
          <FilterSection />
        </div>
                    ...
                    <div className="relative h-32 xs:h-40 sm:h-48 md:h-56 lg:h-64">
                      <img
                        ...
                        onError={(e) => {
                          ...
                          e.target.src = "https://placehold.co/300x300"; // Updated placeholder URL
                        }}
                      />
                      {product.discount && (
